...
def test_config_filepath_environ() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with patch.dict(os.environ, {"CONFIG__FILEPATH": env_filepath}):
            conf = SomeConfig(
                config_values={"field": 1, "some": {"field": 2}},
                config_filepath=filepath,
            )
...
def test_config_filepath_specific_environ() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with patch.dict(
            os.environ,
            {
                "CONFIG__FILEPATH": env_filepath,
                "CONFIG__SOMECONFIG__FILEPATH": specific_env_filepath,
            },
        ):
            conf = SomeConfig(
                config_values={"field": 1, "some": {"field": 2}},
