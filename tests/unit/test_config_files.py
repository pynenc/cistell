...
from cistell.base import ConfigBase
...
def test_pyproject_toml() -> None:
    PYPROJECT_TOML = """
[tool.config]
test_field = "toml_value" # Default value for any field called test_field

[tool.config.grandpa]
test_field = "toml_grandpa_value" # The grandpa config subclass specify toml_grandpa_value

[tool.config.parent]
test_field = "toml_parent_value" # The parent (inheriteds grandpa) but set a new value

# No child config, it inherits from parent/grandpa, we expected parent:toml_parent_value
"""
